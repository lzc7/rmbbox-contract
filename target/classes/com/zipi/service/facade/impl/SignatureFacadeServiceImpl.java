package com.zipi.service.facade.impl;
...
import com.zipi.enums.bbt.CertificateType;
import com.zipi.enums.core.FundRecordStatus;
import com.zipi.service.*;
import com.zipi.service.facade.SignatureFacadeService;
import com.zipi.utils.DateUtils;
import com.zipi.utils.FddClientBase;
import com.zipi.utils.FddConstantCommon;
...
//    @Override
    ...
    public Signature uploadDocs(String fildId, String[] signKeywords) throws Exception{
        ...
        /**上传法大大*/
        String response = FddClientBase.invokeUploadDocs(contractId, DOC_TITLE, file, "", DOC_TYPE, timeStamp);
    ...
    public void signatureDocs(Signature signature, String[] costomerNo, String[] signKeywords, String[] clientType) {
        ...
        if (!FundRecordStatus.SUCCESSFUL.name().equals(signature.getTradeStatus().name())){
            for (int i = signature.getSealNo(); i <= costomerNo.length; i++){//每条协议需要签章三次
                ...
                /**法大大签章*/
                String response = FddClientBase.invokeExtSignAuto("", clientType[i-1], "1",
                        signature.getContractId(), DOC_TITLE, sign_keyword, timeStamp, customer_no);
...
//
    ...
    public void archiveContract(String contractId){
        ...
        String response = FddClientBase.invokeContractFilling(contractId);
        Map result = JSON.parseObject(response, Map.class);
        if (!"1000".equals(result.get("code"))){
...
//
    ...
    public void signatureComposite(CoreSealFlie coreSealFlie) throws Exception{
        ...
        if (SealFileType.BillLoanAgreementForLoan.name().equals(coreSealFlie.getFileType())
...
//            personKey = cacheManagerService.get(REGISTER_CA+coreSealFlie.getInvestId());
//            signKeywords = new String[]{"邦帮堂盖章处","借款公司盖章处","出借方盖章处"};
//            if(user.isEnterprise()){//企业用户
...
//            }
        }else if (SealFileType.CarLoanMediatorContract.name().equals(coreSealFlie.getFileType())
...
//            costomerNo = new String[]{FddConstantCommon.BBT_CUSTOMER_NO.getValue(),loanerId,compensatoryId};
...
//
//        }
//        String response = FddClientBase.invokeSyncPersonAuto(coreUser.getUserName(),null,coreUser.getIdNumber(),identType,coreUser.getUserMobile());
//        Map result = JSON.parseObject(response, Map.class);
//        if (!"1000".equals(result.get("code"))){
//            log.error("法大大申请人个CA异常：{}", response);
//            throw new RuntimeException("个人申请CA异常");
...
//
    public String registerCA(String userName,String idNumber,String userMobile,String userId){
        ...
        String response = FddClientBase.invokeSyncPersonAuto(userName,null,idNumber,identType,userMobile);
        Map result = JSON.parseObject(response, Map.class);
        if (!"1000".equals(result.get("code"))){
            log.error("法大大申请人个CA异常：{}", response);
            throw new RuntimeException("个人申请CA异常");
