...
import com.zipi.utils.FileUtils;
...
public class ContractController {
    ...
    public void getInvestAgreement(@PathVariable("investId") String investId,HttpServletResponse response) {
        ...
        try {
            ...
            byte[] file=null;
            //文件的权限验证
            if(LoanRequestTypes.P_J_B.equals(invest.getRequestType()) || LoanRequestTypes.S_P_J_B.equals(invest.getRequestType())){
                //票据标
                file=sealAutoPdfImpl.getLoanAgreement(investId,invest.getLoanId(),invest.getUserId(),SealFileType.BillLoanAgreementForLend);
            }else if(LoanRequestTypes.C_D_B.equals(invest.getRequestType())){
                //车贷标
    ...
    public void  getLoanAgreementForMG(@PathVariable("loanId") String loanId,HttpServletResponse response){
        ...
        try {
            if(null!=requestTypes){
                if(LoanRequestTypes.P_J_B.equals(requestTypes) || LoanRequestTypes.S_P_J_B.equals(requestTypes)){
                    file=sealAutoPdfImpl.getLoanAgreement(null,loanId,null, SealFileType.BillLoanAgreementForLoan);
                }else if(LoanRequestTypes.C_D_B.equals(requestTypes)){
    ...
    public void test(){
        ...
        String apiId = FddConstantCommon.API_ID.getValue();
        String APP_SECRET = FddConstantCommon.APP_SECRET.getValue();
        String API_URL = FddConstantCommon.API_URL.getValue();
        String BBT_CUSTOMER_NO = FddConstantCommon.BBT_CUSTOMER_NO.getValue();
        String V = FddConstantCommon.V.getValue();
